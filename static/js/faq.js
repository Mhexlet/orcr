window.addEventListener('load', () => {
    ...
    $('.faq-form-submit').on('click', () => {
        ...
        let question = $('.faq-question-input').val();
        ...
        if(validated) {
            $.ajax({
                method: "post",
                url: "/create_question/",
                data: {csrfmiddlewaretoken: token, 'g-recaptcha-response': $('#g-recaptcha-response').val(), name: name, question: question},
                success: (data) => {
                    let notificationText = '';
                    if(data['result'] == 'ok') {
                        notificationText = 'Ваш вопрос успешно отправлен!';
